...
## How UAC differs from other SIP connections

With a typical credential-based SIP connection, your PBX registers to Telnyx. With a UAC Connection, Telnyx registers to your PBX as an extension. IP- and FQDN-authenticated connections use different authentication and routing mechanisms; they are not examples of this reversed registration flow.

During registration, Telnyx is the SIP client and your PBX is the registrar. These roles describe the registration transaction, rather than fixed roles for every SIP request.
...
2. Allowlist Telnyx signaling IPs — your PBX must accept SIP REGISTER and INVITE requests from Telnyx's IP ranges.
   ...
   1. For SIP UAC registrations, Telnyx sends REGISTER requests from the Telnyx UAC IP for your selected region. See the [UAC IP table](#telnyx-uac-ip-addresses) at the end of this article. Allowlist the relevant IP on your PBX/firewall for SIP signaling.
...
2. Select the UAC (User Agent Client) tile.
...
* **Telnyx → PBX:** External UAC settings provide the PBX address and credentials used for registration and outbound requests.
* **PBX → Telnyx:** Internal UAC settings identify the connection and specify the Telnyx application destination in **Internal SIP URI**.
...
| **SIP Subdomain** | Read-only | Auto-generated domain identifying this connection (for example, `qqffqgf1rbok.sip.telnyx.com`). The Receive SIP Subdomain calls setting controls who can call this domain. |
...
| **Internal SIP URI** | Yes | Telnyx destination for calls from your PBX. Use `<extension>@<applicationFQDN>.sip.telnyx.com`, for example `1012@assistant-74907225-9ab8-40c8-96e2-311fd7099ca8.sip.telnyx.com`. Do not include the `sip:` prefix. |
...
| **Expiration (sec)** | No | Requested registration expiry in seconds. Your PBX may negotiate a different expiry, so this is not an exact refresh schedule. |
| **Transport** | No | UDP, TCP, or TLS. **Must match your PBX configuration.** |
| **User-Agent** | No | Optional custom SIP User-Agent for requests to your PBX. Up to 256 characters. See Custom User-Agent Header below for availability, validation, and API reset instructions. |

Click Next.

## Custom User-Agent Header

Use `external_uac_settings.user_agent` to configure a custom SIP User-Agent for the external PBX connection. This can help identify traffic in PBX logs or satisfy a PBX policy that checks the header.

When available for your account, the **User-Agent** field appears under **External UAC settings** in the Portal. If the field is not visible, use the API instructions below.

### Supported values

* Maximum length: **256 characters**, inclusive.
* The API accepts ASCII letters (`a-z`, `A-Z`), numbers (`0-9`), spaces, and `.`, `-`, `_`, `(`, `)`, `/`, `,`.
* The Portal currently accepts the same characters **except commas**. To configure a value containing a comma, use the API.
* The API rejects an empty string (`""`). Send `null` to clear a custom value; omit the field on PATCH to keep the existing value.

### Set a custom value

Send `PATCH /v2/uac_connections/{id}` with this JSON body:

```json
{
  "external_uac_settings": {
    "user_agent": "MyCompany-Telnyx-Bot/1.0"
  }
}
```

The configured header value is:

```text
User-Agent: MyCompany-Telnyx-Bot/1.0
```

### Clear a custom value

Send `PATCH /v2/uac_connections/{id}` with:
...
This removes the custom override so the connection uses its default User-Agent behavior. An omitted field does not clear a previously saved value.

Allow time for configuration changes to propagate before checking new SIP traffic in your PBX logs. Saving an API value does not confirm that it is already present in a SIP message.
...
To edit your configuration later, navigate to **Voice → SIP Trunking**, select your UAC connection, and go to the **UAC Settings** tab.
...
3. Click Edit to update editable fields in Step 2 — for example, changing the Password after a PBX credential rotation, updating the Proxy address after a network change, or adjusting the Expiration (sec) value.
...
* Confirm no firewall is blocking SIP traffic from Telnyx UAC IP, see the [UAC IP](#telnyx-uac-ip-addresses) table above.
...
* If media (audio) isn't flowing, confirm your firewall allows RTP from the same UAC IP (Telnyx uses the same IP for signaling and media on UAC connections). See [UAC IP table](#telnyx-uac-ip-addresses).
...
## Custom User-Agent not appearing on PBX

* Retrieve the connection with `GET /v2/uac_connections/{id}` and check `external_uac_settings.user_agent` to confirm the saved value.
* Allow time for configuration changes to propagate, then inspect new SIP traffic sent to your PBX.
* Use 256 characters or fewer and the supported characters listed above. Values containing commas must currently be configured through the API.
* To remove a custom value, PATCH it to `null`; do not send an empty string.
