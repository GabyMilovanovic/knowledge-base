...
const pendingIds=new Set(["4404575","5377454"]);
for(const route of routes){
 ...
 .on('meta[name="robots"]',{element(e){if(e.getAttribute("content")?.includes("noindex"))noindex++;}})
...
if((xml.match(/<loc>/g)??[]).length!==routes.length)failures.push("Sitemap is missing pages");
