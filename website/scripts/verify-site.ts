...
for(const route of routes){
 const expectedRobots=articles.find(a=>route===`/en/articles/${a.slug}`)?.robots ?? "noindex,follow";
 const html=fs.readFileSync(path.join(dist,route==="/"?"index.html":route.slice(1)),"utf8");
 const ids=new Set<string>();let h1=0,canonical=0,description=0,noindex=0,bodyText="";
 ...
 .on("h1",{element(){h1++;}})
 .on('link[rel="canonical"]',{element(e){canonical++;if(e.getAttribute("href")!==`https://d27az1l5lty0u1.cloudfront.net${route}`)failures.push(`Wrong canonical ${route}`);}})
 ...
 .on('meta[name="robots"]',{element(e){if(e.getAttribute("content")?.includes("noindex"))noindex++;if(e.getAttribute("content")!==expectedRobots)failures.push(`Wrong robots directive ${route}`);}})
 ...
 if(h1!==1||canonical!==1||description!==1||noindex!==1)failures.push(`Invalid initial metadata/H1: ${route}`);
...
for(const a of articles){const included=xml.includes(`<loc>https://d27az1l5lty0u1.cloudfront.net/en/articles/${a.slug}</loc>`);if(included===Boolean(a.robots?.includes("noindex")))failures.push(`Wrong sitemap inclusion ${a.slug}`);}
